...
namespace declang.Expressions
{
  ...
  {
    public And(IExpression leftOperand, IExpression rightOperand)
    ...
    public override IExpressionResult Evaluate(Thing context)
    {
      ...
      if (left.Value.Equals(Truth.FALSE))
      {
        result = new ExpressionResult(this.GetType().Name, ExpressionType.Truth, Truth.FALSE, new List<IExpressionResult>() { left });
        return result;
      }

      IExpressionResult right = RightOperand.Evaluate(context).As(ExpressionType.Truth);

      if (right.Value.Equals(Truth.FALSE))
      {
